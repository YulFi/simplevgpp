...
uniform vec2 u_a, u_b;       // Line endpoints
uniform float u_lineWidth;   // Inner line width
uniform float u_aa;          // Antialiasing
...
void main() {
    float dist = sdSegment(fragPos, u_a, u_b);

    float halfInner = 0.5 * u_lineWidth;
    float halfOuter = halfInner + u_outlineWidth;
